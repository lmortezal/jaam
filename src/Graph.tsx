...
export function Graph({
  ...
  const [groupBox, setGroupBox] = useState<{ x2: number; y2: number } | null>(
    null,
  );
  ...
  async function persist(graph: cytoscape.Core) {
    ...
    try {
      await callbacks.current.onSave(diagramSnapshot(graph));
      if (sequence === saveSequence.current) setSaveState("Diagram saved");
    } catch (e) {
      if (sequence === saveSequence.current)
        setSaveState(`Not saved: ${String(e)}`);
    }
  }
  useEffect(() => {
    ...
    const graph = cytoscape({
      ...
      style: [
        ...
        },
        {
          ...
          style: { "border-color": "#d1c4ff", "border-width": 3 },
        },
    ...
    graph.on("tap", 'node[kind="component"]', (e) => {
      ...
      if (!event?.metaKey && !event?.ctrlKey && !event?.shiftKey)
        callbacks.current.onSelect(e.target.id());
    });
    graph.on("select unselect", 'node[kind="component"]', () => {
      ...
      queueMicrotask(() => {
        ...
        const ids = new Set<string>(
          graph
            .nodes(":selected")
            .filter((n) => n.data("environment") === environment)
            .map((n) => n.id()),
        );
    ...
    graph.on("render", () => {
      ...
      if (node.length) {
        const b = node.renderedBoundingBox({ includeLabels: false });
        setGroupBox({ x2: b.x2, y2: b.y2 });
      }
    });
  ...
  useEffect(() => {
    ...
    if (
      ...
      !data.diagram_views[environment]?.groups.some(
        (g) => `__group__${g.id}` === activeGroupRef.current,
      )
    ) {
      setActiveGroup("");
      setGroupBox(null);
  ...
  useEffect(() => {
    ...
    const visible = new Set(
      graphElements(data, environment, filters).map((e) => e.data.id),
    );
    graph.elements().forEach((e) => {
      e.toggleClass("hidden", !e.hasClass("group") && !visible.has(e.id()));
    });
    graph.nodes(".group").forEach((g) => {
      g.toggleClass("group-active", g.id() === activeGroup);
    });
    synchronizing.current = true;
    graph
      .nodes()
  ...
  return (
    <div className="graph-wrap">
      <div className="graph-toolbar">
        <div className="diagram-save" role="status">
          {saveState}
          {saveState.startsWith("Not saved") && (
            <button onClick={() => cy.current && void persist(cy.current)}>
              Retry save
            </button>
          )}
        </div>
        <div className="graph-actions">
          <button className="secondary" onClick={() => onEditGroup(null)}>
            <FolderPlus size={16} />
            New group
          </button>
          {(data.diagram_views[environment]?.groups.length || 0) > 0 && (
            <select
              aria-label="Diagram group"
              value={activeGroup}
              onChange={(e) => setActiveGroup(e.target.value)}
            >
              <option value="">Select group</option>
              {data.diagram_views[environment].groups.map((g) => (
          ...
          {activeGroup && (
            <button
              className="secondary"
              onClick={() => onEditGroup(activeGroup.replace(/^__group__/, ""))}
            >
              <Pencil size={16} />
              Edit group
            </button>
          ...
          <button
            className="secondary"
            title="Arrange the full diagram. Groups containing locked nodes stay in place."
            onClick={() => {
              const graph = cy.current;
              if (!graph) return;
              layoutGraph(graph);
          ...
          {selected && (
            <button
              ...
              onClick={() => {
                const graph = cy.current;
                if (!graph) return;
                const n = graph.getElementById(selected);
                locked ? n.unlock() : n.lock();
                n.toggleClass("pinned", !locked);
                setLocked(!locked);
                void persist(graph);
              }}
            >
              {locked ? <Unlock size={16} /> : <LockKeyhole size={16} />}{" "}
      ...
      <div className="graph-viewport">
        ...
        />
        {activeGroup && groupBox && (
          <button
            className="group-resize"
            aria-label="Resize diagram group"
            title="Drag to resize; members remain enclosed"
            style={{ left: groupBox.x2 - 12, top: groupBox.y2 - 12 }}
            onPointerDown={(e) => {
              const graph = cy.current,
                node = graph?.getElementById(activeGroup);
            ...
            onPointerMove={(e) => {
              const start = resizing.current,
                node = cy.current?.getElementById(activeGroup);
              if (!start || !node?.length) return;
              node.data({
                minWidth: Math.min(
                  100000,
                  Math.max(
                    100,
                    start.width + (e.clientX - start.x) / start.zoom,
                  ),
                ),
                minHeight: Math.min(
                  100000,
                  Math.max(
                    80,
                    start.height + (e.clientY - start.y) / start.zoom,
                  ),
                ),
              });
            }}
            onPointerUp={() => {
              if (resizing.current && cy.current) {
                resizing.current = null;
                void persist(cy.current);
              }
            }}
            onPointerCancel={() => {
              if (resizing.current && cy.current) {
