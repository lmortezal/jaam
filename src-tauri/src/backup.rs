...
pub fn encrypt(document: &Document, password: String) -> Result<Vec<u8>, String> {
    ...
    if plaintext.len() as u64 > MAX_PLAIN {
...
pub fn decrypt(bytes: &[u8], password: String) -> Result<Document, String> {
    let password = Zeroizing::new(password);
    if bytes.len() > MAX_FILE || password.len() > 1024 {
        return Err("Backup or password exceeds limits".into());
    }
    let decryptor = age::Decryptor::new(bytes).map_err(|_| FAILURE)?;
    if !decryptor.is_scrypt() {
        return Err("Expected a password-encrypted OpsPortal backup".into());
    }
    ...
    reader
        ...
        .map_err(|_| FAILURE)?;
    if plaintext.len() as u64 > MAX_PLAIN {
        return Err("Decrypted backup is too large".into());
    }
